...
#define DF_CANBUS_UNSKT_FILE_PATH  "/tmp/canbusUxSkt.ipc"
...
int main(void) {
    ...
    strcpy(server.sun_path , DF_CANBUS_UNSKT_FILE_PATH ) ;
    ...
    //while(1) {
        ...
        if ( fpid < 0 ) {
            ...
            **send() > 寫資料*/
                    ...
                    fp2 = fopen( "parsedata2.txt" , "w" ) ;//開啟檔案result.txt
                    ...
                    fp3 = fopen( "parsedata2.txt" , "r" )  ; //開啟parsedata.txt
                    fp4 = fopen( "CanBus_SendLog.txt" , "w" ) ;
            ...
            **recv() 讀取資料*/
                while(1) {
                    ...
                    int val = recv( sock2 , buf , sizeof(buf) , 0) ;
                    if( val < 0 ) {
